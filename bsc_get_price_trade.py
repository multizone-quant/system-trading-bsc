...
# swap_info = {'from':from_token, 'to':to_token, 'from_amount':swap_info['amount'], 'to_amount':out_amount}
# 매수/매도 함수가 틀림

# 매수 busd -> sps 
#   Swapping 5.73235 BUSD for 10 SPS
#   Function: swapTokensForExactTokens(uint256 amountOut, uint256 amountInMax, address[] path, address to, uint256 deadline)
#    0	amountOut	uint256	10000000000000000000
#    1	amountInMax	uint256	5760925700910172081
#    2	path	address[]	e9e7cea3dedca5984780bafc599bd69add087d56 -> busd 
#                           bb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c -> bnb
#                           1633b7157e7638c4d6593436111bf125ee74703f -> sps

def token_swap_bsc(exchange, swap_info, pair, buy_sell = 'sell'):
    if buy_sell == 'sell' :
        in_amount = swap_info['from_amount']
        out_amount = swap_info['to_amount']
        min_amount = int(out_amount * (1-SLIPPAGE))
    else :
        out_amount = swap_info['to_amount']
        in_amount = swap_info['from_amount']
        max_amount = int(in_amount * (1+SLIPPAGE))

    deadline = int(time.time()) + 20 # deadline 20초    

    if buy_sell == 'sell' : # ExactTokens : sell
        tx = exchange.functions.swapExactTokensForTokens(in_amount, min_amount, pair, my_addr, deadline).buildTransaction({
                    'gas': GAS,
                    'gasPrice': w3.toWei(GAS_PRICE, 'gwei'),
                    'nonce':  w3.eth.getTransactionCount(my_addr),
                })
    else : # buy
        tx = exchange.functions.swapTokensForExactTokens(out_amount, max_amount, pair, my_addr, deadline).buildTransaction({
                    'gas': GAS,
                    'gasPrice': w3.toWei(GAS_PRICE, 'gwei'),
                    'nonce':  w3.eth.getTransactionCount(my_addr),
                })
...
if 1:
    ...
    while(1) :
        for each in trading_pair :
            ...
            price = ret['to_amount'] / ret['from_amount'] 
            if price > 1.63 : # 1.63이면 지정한 수량 매도
                token_swap_bsc(exchange, ret, pair, 'sell')
                
            pair = get_pair_info(each, 1)
            ret = get_swap_amount_in(exchange, each, pair) # for 매수
            price = ret['from_amount'] / ret['to_amount']  # price = #sps / #busd
            if price <= 0.510 : # 0.510면 지정한 수량 매수
                token_swap_bsc(exchange, ret, pair, 'buy') # 
        print('')
        time.sleep(10)
